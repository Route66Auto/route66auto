...
const Card = () => {
    ...
    return (
        <div>
            <Headers />
            <section className='bg-[url("../../public/images/banner/card.jpg")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                <div className='absolute left-0 top-0 w-full h-full bg-[#2422228a]'>
                    <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                        <div className='flex flex-col justify-center gap-1 items-center h-full w-full text-white'>
                            <h2 className='text-3xl font-bold'>Route 66 Auto</h2>
                                ...
                                <div className='pr-3 md-lg:pr-0'>
                                    <div className='flex flex-col gap-3'>
                                        <div className='bg-white p-4'>
                                            <h2 className='text-md text-green-500 font-semibold'>Produtos em Estoque: {card_products.length}</h2>
                                        </div>
                                        {
                                                    ...
                                                    p.products.map((pt, i) => <div className='w-full flex flex-wrap'>
                                                        ...
                                                        <div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
                                                            <div className='pl-4 sm:pl-0'>
                                                                <h2 className='text-lg text-orange-600'>R$ {pt.productInfo.price - Math.floor((pt.productInfo.price * pt.productInfo.discount) / 100)} </h2>
                                                            ...
                                                            <div className='flex gap-2 flex-col'>
                                                                ...
                                                                <button onClick={() => dispatch(delete_card_product(pt._id))} className='px-5 py-[3px] bg-red-500 text-white'>Remover</button>
                                                            </div>
                                                        </div>
                                                    </div>)
                                                }
                                        ...
                                        }
                                        {
                                            outofstock_products.length > 0 && <div className='flex flex-col gap-3'>
                                                <div className='bg-white p-4'>
                                                    <h2 className='text-md text-red-500 font-semibold'>Produtos fora de estoque:  {outofstock_products.length}</h2>
                                                </div>
                                                <div className='bg-white p-4'>
                                                    {
                                                        outofstock_products.map((p, i) => <div key={i} className='w-full flex flex-wrap'>
                                                            ...
                                                            <div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
                                                                <div className='pl-4 sm:pl-0'>
                                                                    <h2 className='text-lg text-orange-600'>R$ {p.products[0].price - Math.floor((p.products[0].price * p.products[0].discount) / 100)} </h2>
                                                                ...
                                                                <div className='flex gap-2 flex-col'>
                                                                    ...
                                                                    <button onClick={() => dispatch(delete_card_product(p._id))} className='px-5 py-[3px] bg-red-500 text-white'>Remover</button>
                                                                </div>
                                                            </div>
                                                        </div>)
                                                    }
                                ...
                                <div className='pl-3 md-lg:pl-0 md-lg:mt-5'>
                                    {
                                        card_products.length > 0 && <div className='bg-white p-3 text-slate-600 flex flex-col gap-3'>
                                            ...
                                            <div className='flex gap-2'>
                                                <input className='w-full px-3 py-2 border border-slate-200 outline-0 focus:border-green-500 rounded-sm' type="text" placeholder='Inserir cupom' />
                                                <button className='px-5 py-[1px] bg-blue-500 text-white rounded-sm uppercase text-sm'>Aplicar</button>
                                            </div>
                                            <div className='flex justify-between items-center'>
                                                <span>Total</span>
                                                <span className='text-lg text-orange-600'>R$ {price + shipping_fee}</span>
                                            </div>
                                            <button onClick={redirect} className='px-5 py-[6px] rounded-sm hover:shadow-orange-500/20 hover:shadow-lg bg-orange-600 text-sm text-white uppercase'>Ir para o checkout {buy_product_item}</button>
                                        </div>
                                    }
                                </div>
                            </div>
                        </div> : <div>
                            <Link className='px-4 py-1 bg-orange-600 text-white' to='/shops'>Comprar agora</Link>
                        </div>
                    }
                </div>
            </section>
